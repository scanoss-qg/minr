...
void insert(char *token, struct T_TrieNode *nodo,char* algorithmName, unsigned short coding){
	//
	if(token[0]=='\0') {
		sprintf(nodo->algorithmName,"%s",algorithmName);
		nodo->type=1;
		nodo->coding=coding;
		return;
	}

	unsigned char indice=indexOf(token[0]);
	if((indice<0)||(indice >40)) {
		return;
	}
	if(nodo->nodos[indice]==0){
		struct T_TrieNode *nuevo;
		nuevo=(struct T_TrieNode *) malloc (sizeof (struct T_TrieNode));
		for(int i =0;i<40;i++)
				nuevo->nodos[i]=0;
		nuevo->ocurrences=0;
		nuevo->type = -1;
		nodo->nodos[indice]=nuevo;
	}
	insert(&token[1],nodo->nodos[indice],algorithmName,coding);
...
void toLower(char *strToLow){
 ...
 /**
 * @brief Load algotithm definitions from a file
